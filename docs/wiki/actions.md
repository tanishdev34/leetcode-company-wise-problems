...
| Action | Auth | Params | Returns | Called By | Mutates |
|--------|------|--------|---------|-----------|---------|
| `createRoadmap(input)` | Yes | `{ prompt, companyId?, topicSlug?, deadline?, intensity? }` | `{ id, itemCount, feasibility, summary }` | [[components#roadmapcreate]] `RoadmapCreateDialog` | Yes — creates a draft [[data-model#roadmap]] with `generationStatus: "running"`, records `ai_generation_started`, then schedules fast background generation with Next `after()` |
...
| `updateRoadmapItemStatus(itemId, completed)` | Yes | `itemId: string, completed: boolean` | `{ success: true }` | [[components#roadmapview]] `RoadmapView` | Yes — toggles item `completed`/`planned`, upserts [[data-model#userquestion-user-progress]] only when checking complete, creates event |
| `completeRoadmapItem(itemId)` | Yes | `itemId: string` | `{ success: true }` | Compatibility wrapper | Yes — calls `updateRoadmapItemStatus(itemId, true)` |
...
`createRoadmap` validates the prompt (non-empty, max 1000 chars) and returns immediately after creating the draft roadmap, so the modal can close and `RoadmapView` can show the animated background-designing card. The scheduled `generateRoadmapInBackground()` worker calls `generateAiRoadmapPlan()` from `lib/roadmap-ai-planner.ts`, which builds deterministic context via `lib/roadmap-planning-context.ts`, asks OpenRouter for one compact candidate-ID selection with a 60-second timeout, generates the dated schedule locally, validates output via `lib/roadmap-plan-validator.ts`, then persists the plan and marks `generationStatus: "done"`. `buildRoadmapPlanningContext()` uses raw SQL for the candidate list, solved count, and due-review count to avoid loading full user progress/review collections and nested company relations into JS; it now retrieves a larger candidate pool for roadmap planning. The hybrid planner parses explicit deadline fields and prompt deadlines like "before August" or "1st of August", uses intensity-based cadence, applies the AI-selected question subset/order when available, fills any gaps locally, falls back to deterministic difficulty interleaving when AI selection is slow or invalid, adds due reviews when available, and creates templated day themes/reasons without asking the model to emit the full schedule. `itemType: "new"` is still normalized to `"new_question"` in `lib/roadmap-ai-schemas.ts` for compatibility with older imported plans. Failures mark `generationStatus: "error"`, persist `generationError`, and record a `generation_failed` event.

Roadmap generation logs coarse timings to the server console using `[roadmap:<id>]` prefixes. Current stages include `draft:create`, `background:start`, `context:queries`, `context:map`, `planner:ai-select` or `planner:ai-select-fallback`, `planner:hybrid-plan`, `planner:validate`, `persist:complete`, and `background:complete` / `background:failed`.
